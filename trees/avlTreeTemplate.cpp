...
struct Node *rightRotate(struct Node *y)
{
    ...
    if(y->minGap == -1)
        cout<<"rr here y "<<y->key<<endl;;
    if(x->minGap == -1){
        cout<<"rr here x "<<x->key<<endl;;
        cout<<"rr here x ka y "<<y->key<<endl;;
    }
    // Return new root
    return x;
}
...
int getMaxGap(struct Node* root, int x, int y)
{
    return y - x;
}

int getMinGap(struct Node* root, int x, int y)
{
    //completely overlap
    if(root->mn >= x && root->mx <= y)
    {
        if(x == y)
        {
            return INT_MAX;
    ...
    if(x == root->key)
    {
        return min(root->right->mn - x , getMinGap(root->right,root->right->mn,y));
    }
    if(y == root->key)
    {
        return min(y - root->left->mx , getMinGap(root->right,x,root->right->mx));
    }

    //no overlap
    else if(root->right != NULL && root->right->mn<x)
    {
        return getMinGap(root->right,x,y);
    }
    //no overlap
    else if(root->left != NULL && root->left->mx>y)
    {
        return getMinGap(root->left,x,y);
    }
    //partially overlap
    else
    {
        return min((root->left!=NULL?getMinGap(root->left,x,y):INT_MAX),
                   (root->right!=NULL?getMinGap(root->right,x,y):INT_MAX));
    }
}
...
//    cout<<"maxgap range: "<<getMaxGap(root,0,11)<<endl;
//    cout<<"mingap range: "<<getMinGap(root,-5,19)<<endl;
//    return 0;
//}
